...
class DataLoaderFactory:
    ...
    def get_all_dataloaders(self):
        ...
        for split in SPLIT_KEYS:
            dataset = DatasetFactory(self.config, split)
    ...
    def get_class_distribution(self):
        """Returns class distribution (useful for stratification)."""
        class_counts = {}
        for split in SPLIT_KEYS:
            dataset = DatasetFactory(self.config, split)
    ...
    def collate_fn(self, batch):
        """Custom function to merge samples into a batch."""
        return torch.utils.data.dataloader.default_collate(batch)

    def worker_init_fn(self, worker_id):
        """Ensures each worker gets a different random seed."""
        seed = self.config["loader"].get("seed", 42)
